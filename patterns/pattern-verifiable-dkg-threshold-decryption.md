...
2. [operator] Each member generates a PVSS share and publishes it with a zero-knowledge proof of correctness. Shares are aggregated into the committee public key.
...
6. [operator] Each committee member produces a decryption share with a per-share zero-knowledge proof; shares are aggregated once the threshold is met.
